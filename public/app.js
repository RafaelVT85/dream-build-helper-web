...
function renderDiabloBuildDetails(buildId) {
  ...
  const kvList = (obj) => Object.entries(obj || {}).map(([k, v]) =>
        `<li><b>${escapeHtml(k)}:</b> ${Array.isArray(v) ? v.map(escapeHtml).join(", ") : escapeHtml(String(v))}</li>`
                                                          ).join("");
  ...
  if (build._phase === "leveling") {
        ...
        bodyHtml = `
              ${rows ? `
                      <div class="section-label">Progressão de skills (nível a nível)</div>
                              <table class="diablo-table"><thead><tr><th>Nível</th><th>O que fazer</th></tr></thead><tbody>${rows}</tbody></table>
                                    ` : ""}
                                          ${build.milestones?.length ? `
                                                  <div class="section-label">Marcos de Encantamento</div>
                                                          <ul class="notes-list">${build.milestones.map(m => `<li><b>Nv.${escapeHtml(String(m.level))}:</b> ${escapeHtml(m.action)}</li>`).join("")}</ul>
                                                                ` : ""}
                                                                      ${build.skill_bar?.length ? `<div class="section-label">Barra de skills</div><ul class="notes-list">${build.skill_bar.map(s => `<li>${escapeHtml(s)}</li>`).join("")}</ul>` : ""}
                                                                            ${(build.enchantment_slot_1 || build.enchantment_slot_2) ? `
                                                                                    <div class="section-label">Encantamentos</div>
                                                                                            <ul class="notes-list">
                                                                                                      ${build.enchantment_slot_1 ? `<li>Slot 1: ${escapeHtml(build.enchantment_slot_1)}</li>` : ""}
                                                                                                                ${build.enchantment_slot_2 ? `<li>Slot 2: ${escapeHtml(build.enchantment_slot_2)}</li>` : ""}
                                                                                                                        </ul>` : ""}
                                                                                                                              ${build.glyph_priority?.length ? `<div class="section-label">Prioridade de Glyphs</div><p class="notes">${build.glyph_priority.map(escapeHtml).join(" → ")}</p>` : ""}
                                                                                                                                    ${build.note ? `<p class="notes">⚠️ ${escapeHtml(build.note)}</p>` : ""}
                                                                                                                                          ${build.total_points ? `<p class="notes">Total de pontos ao chegar em 70: ${escapeHtml(String(build.total_points))}</p>` : ""}
                                                                                                                                              `;
  } else {
        const skillBarHtml = (build.skill_bar || []).map(s =>
                `<li><b>${escapeHtml(s.skill)}</b>${s.role ? ` — ${escapeHtml(s.role)}` : ""}</li>`
                                                             ).join("");
        const mercHtml = build.mercenary ? `
              <ul class="notes-list">
                      ${build.mercenary.hired ? `<li>Contratado: ${escapeHtml(build.mercenary.hired)}</li>` : ""}
                              ${build.mercenary.reinforcement ? `<li>Reforço: ${escapeHtml(build.mercenary.reinforcement)}</li>` : ""}
                                    </ul>` : "";
        const paragonHtml = build.paragon ? `
              ${build.paragon.points_spent ? `<p class="notes">Pontos gastos: ${escapeHtml(String(build.paragon.points_spent))}</p>` : ""}
                    ${build.paragon.boards_order?.length ? `<p class="notes">Ordem de boards: ${build.paragon.boards_order.map(escapeHtml).join(" → ")}</p>` : ""}
                          <ul class="notes-list">${kvList(build.paragon.glyphs_endgame)}</ul>
                              ` : "";
        const equipHtml = build.equipment ? `<ul class="notes-list">${kvList(build.equipment)}</ul>` : "";
        const prosConsHtml = (build.pros?.length || build.cons?.length) ? `
              <div class="meta-row">
                      ${build.pros?.length ? `<div><b>Prós:</b> ${build.pros.map(escapeHtml).join(", ")}</div>` : ""}
                              ${build.cons?.length ? `<div><b>Contras:</b> ${build.cons.map(escapeHtml).join(", ")}</div>` : ""}
                                    </div>` : "";

      bodyHtml = `
            ${build.main_set ? `<p class="notes"><b>Set:</b> ${escapeHtml(build.main_set)}</p>` : ""}
                  ${build.key_unique ? `<p class="notes"><b>Unique-chave:</b> ${escapeHtml(build.key_unique)}</p>` : ""}
                        ${build.key_mythic ? `<p class="notes"><b>Mítico-chave:</b> ${escapeHtml(build.key_mythic)}</p>` : ""}
                              ${build.critical_requirement ? `<p class="notes">⚠️ <b>Requisito crítico:</b> ${escapeHtml(build.critical_requirement)}</p>` : ""}
                                    ${skillBarHtml ? `<div class="section-label">Barra de skills</div><ul class="notes-list">${skillBarHtml}</ul>` : ""}
                                          ${mercHtml ? `<div class="section-label">Mercenário</div>${mercHtml}` : ""}
                                                ${paragonHtml ? `<div class="section-label">Paragon</div>${paragonHtml}` : ""}
                                                      ${equipHtml ? `<div class="section-label">Equipamento</div>${equipHtml}` : ""}
                                                            ${prosConsHtml}
                                                                `;
  }

  const bossFarmingHtml = (currentGameData.boss_farming || []).map(bf =>
                      ...
                      ${statusPill}
                            ${season ? `<span>Temporada ${escapeHtml(String(season.number))} — ${escapeHtml(season.name)}</span>` : ""}
                                  ${sourceLink}
                                          ...
                                          ${bodyHtml}
                                              ${bossFarmingHtml ? `
                                                    <div class="section-label">Onde farmar boss materials</div>
                                                          <table class="diablo-table"><thead><tr><th>Boss</th><th>Local</th><th>Material</th><th>Dificuldade mín.</th></tr></thead><tbody>${bossFarmingHtml}</tbody></table>
                                                              ` : ""}
                                                                `;
}
