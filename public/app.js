...
// ---------- Suporte a jogos com schema diferente (ex.: Diablo IV) ----------
// A maioria dos jogos usa o schema do Shape of Dreams (travelers/builds/memories/
// essences/constellation). Diablo IV Mago usa um schema próprio (class/leveling_builds/
// endgame_builds/boss_farming) porque a estrutura do jogo é bem diferente — em vez de
// forçar um molde que não cabe, esses jogos têm seu próprio caminho de renderização,
// acionado sempre que o JSON não tem `travelers`.

function isDiabloGame(data) {
        return !!(data && data.class && !data.travelers && (data.leveling_builds || data.endgame_builds));
}

function diabloAllBuilds() {
        const lvl = (currentGameData.leveling_builds || []).map(b => ({ ...b, _phase: "leveling" }));
        const end = (currentGameData.endgame_builds || []).map(b => ({ ...b, _phase: "endgame" }));
        return [...lvl, ...end];
}

function diabloFindBuild(buildId) {
        return diabloAllBuilds().find(b => b.id === buildId) || null;
}

// Selos de "escola" por palavra-chave no nome da skill — o jogo não manda um
// campo de elemento pronto pra Diablo, então adivinhamos pelo nome mesmo.
const DIABLO_SKILL_SCHOOL = [
    { test: /fire|inferno|incinerat|combust|meteor|firewall|flame|burn|scorch/i, emoji: "🔥", color: "#f0a63c" },
    { test: /ice|frost|frozen|glacial|blizzard|cold|winter|hoar/i, emoji: "❄️", color: "#7ecbff" },
    { test: /lightning|charged|chain|spark|static|electrocut|shock/i, emoji: "⚡", color: "#ffe27a" },
    ];
function diabloSkillBadge(name) {
        const found = DIABLO_SKILL_SCHOOL.find(s => s.test.test(name || ""));
        const c = found || { emoji: "🔮", color: "#b389ff" };
        return `<span class="type-badge" style="background:${c.color}22;border-color:${c.color}66;">${c.emoji}</span>`;
}

const DIABLO_EQUIP_ICON = {
        helm: "🪖", chest: "👕", gloves: "🧤", pants: "👖", boots: "🥾",
        ring: "💍", amulet: "📿", weapon: "🪄", offhand: "📖", focus: "🔯",
};
function diabloEquipBadge(key) {
        const k = (key || "").toLowerCase();
        const found = Object.keys(DIABLO_EQUIP_ICON).find(needle => k.includes(needle));
        const emoji = found ? DIABLO_EQUIP_ICON[found] : (k.includes("priority") || k.includes("splinter") ? "📋" : "🛡️");
        return `<span class="type-badge">${emoji}</span>`;
}

// Estrutura de seção inspirada no próprio guia fonte (Maxroll): barra colorida
// + título, corpo full-width — em vez de cards soltos numa grade.
function diabloSection(title, innerHtml) {
        if (!innerHtml) return "";
        const anchorId = `d4sec-${title.normalize("NFD").replace(/[̀-ͯ]/g, "").replace(/[^a-zA-Z0-9]+/g, "-").toLowerCase()}`;
        return `<section class="diablo-section" id="${anchorId}">
                <h4 class="diablo-section-title">${escapeHtml(title)}</h4>
                        <div class="diablo-section-body">${innerHtml}</div>
                            </section>`;
}

// Lista ordenada (1., 2., 3.) com selo — usada pra Soul Splinters, Mythic
// Priority Order, Ordem de Boards: tudo no guia fonte é "trabalhe essa lista
// em ordem", não uma pilha de checkboxes.
function diabloPriorityList(items, badgeFn) {
        if (!items?.length) return "";
        return `<ol class="diablo-priority-list">${items.map((it, i) => `
                <li><span class="diablo-priority-num">${i + 1}</span>${badgeFn ? badgeFn(it) : ""}<span>${escapeHtml(it)}</span></li>
                    `).join("")}</ol>`;
}
...
function diabloProsCons(pros, cons) {
        ...
        return `<div class="diablo-proscons">
                ${pros?.length ? `<div class="diablo-pros"><div class="diablo-proscons-title">✓ Prós</div><ul>${pros.map(p => `<li>✓ ${escapeHtml(p)}</li>`).join("")}</ul></div>` : ""}
                        ${cons?.length ? `<div class="diablo-cons"><div class="diablo-proscons-title">✗ Contras</div><ul>${cons.map(c => `<li>✗ ${escapeHtml(c)}</li>`).join("")}</ul></div>` : ""}
                            </div>`;
}

function renderDiabloTravelerList() {
        const cls = currentGameData.class;
        el.travelerList.innerHTML = `
                <button class="traveler-chip active" disabled title="${escapeHtml(cls.name_pt)} (${escapeHtml(cls.name_en)})">
                              <span class="avatar-fallback" style="background:${colorFor(cls.id)}">${escapeHtml((cls.name_pt || "?").charAt(0))}</span>
                                                  <span>${escapeHtml(cls.name_pt)}</span>
                                                                          </button>`;
}

function renderDiabloBuildCards() {
        const builds = diabloAllBuilds();
        const cls = currentGameData.class;
        el.buildListTitle.textContent = `Builds de ${cls.name_pt}`;
        if (!builds.length) {
                      el.buildList.innerHTML = `<p class="empty-hint">Nenhuma build cadastrada ainda.</p>`;
                      return;
        }
        el.buildList.innerHTML = builds.map(b => `
                <button class="build-card ${b.id === currentBuildId ? "active" : ""}" data-build="${escapeHtml(b.id)}">
                              <div class="build-card-top">
                                                    <span class="build-card-name">${escapeHtml(b.name)}</span>
                                                                                  <span class="tag-pill">${b._phase === "leveling" ? "🌱 Leveling" : "🏆 Endgame"}</span>
                                                                                                                      </div>
                                                                                                                                                                ${b.tier || b.status ? `<div class="build-card-stats"><span>${escapeHtml(b.tier || b.status)}</span></div>` : ""}
                                                                                                                                                                                                              </button>
                                                                                                                                                                                                                                                              `).join("");
...
function renderDiabloBuildDetails(buildId) {
        const build = diabloFindBuild(buildId ?? currentBuildId);
        const cls = currentGameData.class;
        const season = currentGameData.season;

  if (!build) {
              el.buildDetails.innerHTML = `<p class="empty-hint">Escolha uma build acima.</p>`;
              return;
  }
  ...
  // Cada peça de equipamento vira uma linha "slot (pequeno, discreto) → nome
  // do item (em destaque)" — é assim que o guia fonte mostra cada slot do
  // personagem, com o item como protagonista e o slot como legenda.
  const equipmentHtml = (obj) => {
              if (!obj) return "";
              const rows = Object.entries(obj).map(([k, v]) => `
                          <li class="diablo-equip-row">
                                          ${diabloEquipBadge(k)}
                                                          <div>
                                                                              <div class="diablo-equip-slot">${escapeHtml(k.replace(/_/g, " "))}</div>
                                                                                                  <div class="diablo-equip-name">${Array.isArray(v) ? v.map(escapeHtml).join(", ") : escapeHtml(String(v))}</div>
                                                                                                                  </div>
                                                                                                                              </li>`).join("");
              return `<ul class="diablo-equip-list">${rows}</ul>`;
  };

  const sections = [];

  if (build._phase === "leveling") {
              if (build.skill_progression?.length) {
                                  const rows = build.skill_progression.map(sp =>
                                                              `<tr><td class="lvl-cell">${escapeHtml(String(sp.level))}</td><td>${escapeHtml(sp.action)}</td></tr>`
                                                                                                                                        ).join("");
                                  sections.push(diabloSection("Progressão de skills (nível a nível)",
                                                                                      `<table class="diablo-table"><thead><tr><th>Nível</th><th>O que fazer</th></tr></thead><tbody>${rows}</tbody></table>`));
              }
              if (build.milestones?.length) {
                                  const items = build.milestones.map(m => `<li><b>Nv.${escapeHtml(String(m.level))}:</b> ${escapeHtml(m.action)}</li>`).join("");
                                  sections.push(diabloSection("Marcos de Encantamento", `<ul class="notes-list">${items}</ul>`));
              }
              if (build.skill_bar?.length) {
                                  const items = build.skill_bar.map(s => `<li>${diabloSkillBadge(s)} ${escapeHtml(s)}</li>`).join("");
                                  sections.push(diabloSection("Barra de skills", `<ul class="diablo-skill-list">${items}</ul>`));
              }
              if (build.enchantment_slot_1 || build.enchantment_slot_2) {
                                  let items = "";
                                  if (build.enchantment_slot_1) items += `<li>${diabloSkillBadge(build.enchantment_slot_1)} <b>Slot 1:</b> ${escapeHtml(build.enchantment_slot_1)}</li>`;
                                  if (build.enchantment_slot_2) items += `<li>${diabloSkillBadge(build.enchantment_slot_2)} <b>Slot 2:</b> ${escapeHtml(build.enchantment_slot_2)}</li>`;
                                  sections.push(diabloSection("Encantamentos", `<ul class="diablo-skill-list">${items}</ul>`));
              }
              ...
              if (build.main_set || build.key_unique || build.key_mythic || build.critical_requirement) {
                                  let html = "";
                                  if (build.main_set) html += `<p class="notes"><b>Set:</b> ${escapeHtml(build.main_set)}</p>`;
                                  if (build.key_unique) html += `<p class="notes"><b>Unique-chave:</b> ${escapeHtml(build.key_unique)}</p>`;
                                  if (build.key_mythic) html += `<p class="notes"><b>Mítico-chave:</b> ${escapeHtml(build.key_mythic)}</p>`;
                                  if (build.critical_requirement) html += `<p class="notes">⚠️ <b>Requisito crítico:</b> ${escapeHtml(build.critical_requirement)}</p>`;
                                  sections.push(diabloSection("Visão geral", html));
              }
              if (build.skill_bar?.length) {
                                  const items = build.skill_bar.map(s =>
                                                              `<li>${diabloSkillBadge(s.skill)} <b>${escapeHtml(s.skill)}</b>${s.role ? ` — ${escapeHtml(s.role)}` : ""}</li>`
                                                                                                                         ).join("");
                                  sections.push(diabloSection("Barra de skills", `<ul class="diablo-skill-list">${items}</ul>`));
              }
              if (build.mercenary) {
  ...
  const bossFarmingRows = (currentGameData.boss_farming || []).map(bf =>
              `<tr><td>${escapeHtml(bf.boss)}</td><td>${escapeHtml(bf.location)}</td><td>${escapeHtml(bf.material)}</td><td>${escapeHtml(bf.min_difficulty)}</td></tr>`
                                                                                                                                        ).join("");
        if (bossFarmingRows) {
                      sections.push(diabloSection("Onde farmar boss materials",
                                                                  `<table class="diablo-table"><thead><tr><th>Boss</th><th>Local</th><th>Material</th><th>Dificuldade mín.</th></tr></thead><tbody>${bossFarmingRows}</tbody></table>`));
        }

  el.buildDetails.innerHTML = `
        <h3>${escapeHtml(cls.name_pt)} — ${escapeHtml(build.name)}</h3>
                  <div class="meta-row">
                                  ${phasePill}
                                                        ${statusPill}
...
async function selectGame(gameId, { silent }) {
        ...
        el.gameView.classList.toggle("diablo-mode", isDiabloGame(currentGameData));
        renderTravelerList();
        if (isDiabloGame(currentGameData)) {
                      currentBuildId = null;
                      renderBuildCards({ keepSelection: false });
                      const builds = diabloAllBuilds();
                      if (builds.length) selectBuild(builds[0].id, { record: false });
        } else if (currentGameData.travelers?.length) {
...
function renderTravelerList() {
        if (isDiabloGame(currentGameData)) { renderDiabloTravelerList(); return; }
...
function selectTraveler(travelerId, { record, autoPick }) {
        if (isDiabloGame(currentGameData)) return; // esse tipo de jogo não tem conceito de "personagem"
...
function currentTraveler() {
        if (isDiabloGame(currentGameData)) return { id: currentGameData.class.id, name: currentGameData.class.name_pt };
        return currentGameData?.travelers?.find(t => t.id === currentTravelerId) || null;
}

function currentBuild() {
        if (isDiabloGame(currentGameData)) return diabloFindBuild(currentBuildId);
...
function renderBuildCards({ keepSelection }) {
        if (isDiabloGame(currentGameData)) { renderDiabloBuildCards(); return; }
        const traveler = currentTraveler();
        const builds = [...(traveler?.builds || [])].sort(sorter(currentSort));
...
function selectBuild(buildId, { record }) {
        ...
        state.current = {
                      ...
                      travelerId: isDiabloGame(currentGameData) ? (currentGameData.class?.id || null) : currentTravelerId,
...
function renderBuildDetails() {
        if (isDiabloGame(currentGameData)) { renderDiabloBuildDetails(currentBuildId); return; }
        const traveler = currentTraveler();
